...
export default class MeshRenderer2D{
  ...
  draw(){
	...
	obj.items.forEach(item=>{
		...
		if( obj.isItemVisible(item) ){
		  ...
		  obj.updateViewport(item.renderer)
  ...
  updateAttributesAndUniforms(item){
	...
	let transforms = item.renderer.transforms;
	let geometry = item.renderer.geometry;
  ...
  isItemVisible(item){
	...
	*/
	...
	let rect = item.viewnode.getBoundingClientRect();
	let isCovered = false;
	for(let i=obj.items.indexOf(item)+1; i<obj.items.length; i++){
		...
		let higherRect = obj.items[i].viewnode.getBoundingClientRect();
  ...
  get globalColormapRange(){
	...
	return obj.items.reduce((acc,item)=>{
		...
		let v = item.renderer.geometry.domain.v;
